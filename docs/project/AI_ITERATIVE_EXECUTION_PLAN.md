...
--- PRE-FLIGHT CHECK ---
1. Verify F5-C has `[x]` in Estado de ejecución.
--- END PRE-FLIGHT CHECK ---

[TASK — F5-D: Create FUTURE_IMPROVEMENTS.md]

Project root: d:/Git/veterinary-medical-records

Create `docs/project/FUTURE_IMPROVEMENTS.md` — a 2/4/8 week improvement roadmap. This is an explicit deliverable required by the technical exercise evaluators to demonstrate forward-looking engineering thinking.

Use the data below to populate each horizon. Each item must include: what, why (reference to audit/ADR), effort estimate (S/M/L), and risk.

---

## Content for the roadmap

### Week 2 — Quick wins (S effort, low risk)

1. **SQLite WAL mode + busy_timeout PRAGMA**
   - What: Add `PRAGMA journal_mode=WAL` and `PRAGMA busy_timeout=5000` in `database.py`'s connection setup.
   - Why: ADR-ARCH-0002 notes missing WAL for concurrent reads during writes; codebase audit finding #7.
   - Effort: S | Risk: Low

2. **CI coverage threshold gates**
   - What: Add `--cov-fail-under=80` (pytest-cov) and vitest coverage threshold in CI quality job.
   - Why: Codebase audit finding #14 — coverage reports exist but have no enforcement.
   - Effort: S | Risk: Low

3. **Centralize remaining debug env reads**
   - What: Move `PDF_EXTRACTOR_FORCE_ENV` (pdf_extraction.py) and `INTERPRETATION_DEBUG_INCLUDE_CANDIDATES_ENV` (interpretation.py) into `Settings`.
   - Why: 12-Factor audit Factor III — two application modules still read `os.getenv` directly.
   - Effort: S | Risk: Low

4. **Frontend test coverage: `lib/utils.ts` error paths**
   - What: Add tests for `apiFetchJson`/`apiFetchBlob` error branches (24% → 80%+).
   - Why: F4-A audit — utils is a shared dependency; untested error paths affect all API consumers.
   - Effort: S | Risk: Low

5. **Frontend test coverage: `AddFieldDialog.tsx` + `useSourcePanelState.ts`**
   - What: Test save-lock transitions (30% → 70%+) and hook state transitions (45% → 80%+).
   - Why: F4-A audit — critical interaction paths untested.
   - Effort: S | Risk: Low

6. **Add "Known limitations" section to TECHNICAL_DESIGN.md**
   - What: Transparent list of current constraints (single-process, no auth, SQLite write lock, etc.).
   - Why: F5-A audit improvement #4 — evaluators value transparency about what the system does NOT do.
   - Effort: S | Risk: Low

### Week 4 — Structural improvements (M effort, medium risk)

7. **Decompose `routes.py` (1,015 lines) into endpoint groups**
   - What: Split into `api/documents.py`, `api/runs.py`, `api/review.py`, `api/observability.py`, keep `api/routes.py` as router aggregator.
   - Why: Codebase audit finding #12 — single 1K-line file violates the ≤500 LOC guideline.
   - Effort: M | Risk: Medium

8. **Modularize `extraction_observability.py` (995 lines)**
   - What: Split into storage, transform, aggregate units.
   - Why: Codebase audit finding #11 — close to the audit's "file > 700 lines" threshold.
   - Effort: M | Risk: Medium

9. **Streaming/bounded upload guard**
   - What: Add size limit enforcement and streaming parser before full memory allocation in upload route.
   - Why: Codebase audit finding #5 — no upload size guard exists.
   - Effort: M | Risk: Medium

10. **Expand backend failure-path test coverage**
    - What: Add tests for `orchestrator.py` partial-failure transitions, `database.py` lock/retry paths.
    - Why: F4-B audit — orchestrator at 76%, database at 73%.
    - Effort: M | Risk: Low

11. **SourcePanel + UploadDropzone test suites**
    - What: Create comprehensive test suites (SourcePanel at 0%, UploadDropzone at 50%).
    - Why: F4-A audit — P1 frontend coverage holes.
    - Effort: M | Risk: Low

12. **Centralize PdfViewer mock across test files**
    - What: Extract shared PdfViewer mock from 8+ test files into `__tests__/helpers/`.
    - Why: F4-A audit — duplicated mocks cause maintenance burden.
    - Effort: S | Risk: Low

### Week 8 — Architecture evolution (L effort, requires design)

13. **Split `sqlite_document_repository.py` (750 lines) by aggregate**
    - What: Separate into `document_repository.py`, `run_repository.py`, `calibration_repository.py`, each implementing a focused Protocol.
    - Why: ADR-ARCH-0003 risk — repository grows unwieldy beyond ~10 tables.
    - Effort: L | Risk: Medium

14. **Optional Compose worker profile for background processing**
    - What: Add `profiles: [worker]` service in docker-compose.yml reusing scheduler code.
    - Why: 12-Factor Factor VIII — no dedicated worker process type. Requires SQLite→PostgreSQL migration first (concurrent writes).
    - Effort: L | Risk: High (depends on item #17)

15. **Minimal auth/token boundary**
    - What: Add token-based auth middleware for non-localhost deployment.
    - Why: Codebase audit finding #6 — no auth/authz layer exists.
    - Effort: L | Risk: Medium

16. **Persistent event tracing / observability metrics**
    - What: Implement structured event tracing system per TECHNICAL_DESIGN §9.2.
    - Why: Deferred from initial design — needed for production debugging.
    - Effort: L | Risk: Medium

17. **PostgreSQL adapter behind DocumentRepository Protocol**
    - What: Build `PostgresDocumentRepository` implementing the same Protocol. Update composition root.
    - Why: ADR-ARCH-0002 upgrade path — unlocks multi-process/multi-user deployment.
    - Effort: L | Risk: High

18. **Schema migration tooling**
    - What: Replace `ensure_schema()` bootstrap with Alembic or equivalent migration chain.
    - Why: ADR-ARCH-0003 risk — manual schema changes don't scale beyond ~10 tables.
    - Effort: M | Risk: Medium (depends on choosing ORM or staying raw SQL)
...
- Brief intro paragraph: what this document is, how it connects to the codebase audit and ADRs.
- Three sections: `## Week 2 — Quick wins`, `## Week 4 — Structural improvements`, `## Week 8 — Architecture evolution`
- Each item as a row in a table: `| # | Improvement | Source | Effort | Risk |` with a brief description.
- Below the table in each section, expand each item with 2–3 sentences (what, why, acceptance criteria).
- Footer: "This roadmap was generated from audit findings (F1-A, F2-A, F4-A/B, F5-A), architecture decision records (ADR-ARCH-0001 through 0004), and TECHNICAL_DESIGN.md deferred items."
- Cross-reference ADRs and audit docs using relative links.

## Additional tasks

1. Update `docs/README.md` — add FUTURE_IMPROVEMENTS.md to the reading order.
2. Update root `README.md` — ensure the "Future improvements" or similar section links to the new file.
3. Verify all referenced source files and line counts are still accurate (re-check with `wc -l` if needed).
...
STEP A — Commit FUTURE_IMPROVEMENTS.md + doc updates:
1. git add -A -- . ':!docs/project/AI_ITERATIVE_EXECUTION_PLAN.md'
2. git commit -m "docs(plan-f5d): create FUTURE_IMPROVEMENTS.md roadmap

New file: docs/project/FUTURE_IMPROVEMENTS.md
Updated: docs/README.md, README.md

Test proof: <pytest summary> | <npm test summary>"

STEP B — Commit plan update:
1. Mark `- [ ] F5-D` → `- [x] F5-D` in Estado de ejecución.
2. Clean Prompt activo: `### Paso objetivo` → `_Completado: F5-D_`, `### Prompt` → `_Vacío._`
3. git add docs/project/AI_ITERATIVE_EXECUTION_PLAN.md
4. git commit -m "docs(plan-f5d): mark step done"

STEP C — Push:
1. git push origin improvement/refactor

STEP D — Update PR #145:
Run `gh pr edit 145 --body "..."` marking F5-D done in the checklist.
...
The next step is F6-A (Claude — hard-gate, smoke test). Say:
"✓ F5-D completado, CI verde, PR actualizada. Siguiente: abre un chat nuevo en Copilot → selecciona **Claude** → adjunta AI_ITERATIVE_EXECUTION_PLAN.md → escribe Continúa."
