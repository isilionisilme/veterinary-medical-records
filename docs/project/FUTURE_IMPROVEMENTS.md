...
| 3 | ~~Centralize remaining debug env reads in settings~~ **✅ Done (Iteration 11, F18-A)** | [12_FACTOR_AUDIT.md](refactor/12_FACTOR_AUDIT.md), [ADR-ARCH-0001](adr/ADR-ARCH-0001-modular-monolith.md) | S | Low |
...
| 5 | ~~Increase frontend coverage for AddFieldDialog + source panel hook~~ **✅ Done (Iteration 11, F18-R)** | [F4-A in history](implementation/IMPLEMENTATION_HISTORY.md) | S | Low |
...
3. **~~Centralize debug env reads.~~** **✅ Done (Iteration 11, F18-A).** Moved 2 remaining `os.getenv` calls (`PDF_EXTRACTOR_FORCE`, `VET_RECORDS_INCLUDE_INTERPRETATION_CANDIDATES`) into `Settings` dataclass. Zero direct `os.getenv` calls remain outside `settings.py`/`config.py`.
...
5. **~~AddFieldDialog and source-panel state tests.~~** **✅ Done (Iteration 11, F18-R).** SourcePanel and UploadDropzone test depth expanded with branch coverage and interaction edge cases (21 new tests). AddFieldDialog already had 8 tests covering state transitions.
...
| 10 | ~~Expand backend failure-path tests~~ **✅ Done (Iteration 11, F18-Q)** | [codebase_audit.md](refactor/codebase_audit.md) | M | Low |
| 11 | ~~Add SourcePanel and UploadDropzone full test suites~~ **✅ Done (Iteration 11, F18-R)** | [codebase_audit.md](refactor/codebase_audit.md) | M | Low |
| 12 | ~~Centralize PdfViewer mock helpers~~ **✅ Done (Iteration 11, F18-B)** | [FRONTEND_IMPLEMENTATION.md](FRONTEND_IMPLEMENTATION.md) | S | Low |
...
10. **~~Backend failure-path coverage expansion.~~** **✅ Done (Iteration 11, F18-Q).** Added 22 tests for orchestrator partial failures and DB lock/retry resilience. Run-state transitions and error handling covered with deterministic assertions.
11. **~~Frontend interaction suite expansion.~~** **✅ Done (Iteration 11, F18-R).** Comprehensive tests added for SourcePanel and UploadDropzone covering branch coverage and interaction edge cases (21 new tests). Both components now have strong regression protection.
12. **~~Shared PdfViewer mock extraction.~~** **✅ Done (Iteration 11, F18-B).** Extracted shared `__mocks__/react-pdf` auto-mock replacing 9 duplicated local mocks. All viewer-related tests reuse the single helper.

## Week 8 — Architecture evolution

| # | Improvement | Source | Effort | Risk |
| --- | --- | --- | --- | --- |
| 13 | ~~Split SQLite repository by aggregate~~ **✅ Done (Iteration 11, F18-S)** | [ADR-ARCH-0003](adr/ADR-ARCH-0003-raw-sql-repository-pattern.md), [codebase_audit.md](refactor/codebase_audit.md) | L | Medium |
...
13. **~~Repository split by aggregate.~~** **✅ Done (Iteration 11, F18-S).** Split `sqlite_document_repository.py` (751 LOC) into 3 aggregate modules + façade: `sqlite_document_repo.py` (241 LOC), `sqlite_run_repo.py` (302 LOC), `sqlite_calibration_repo.py` (123 LOC), façade (182 LOC). Composition root wires same public API.
...
|---|---|---|---|---|
| 19 | ~~Performance benchmarks: latency baselines for upload, extraction, and query endpoints~~ **✅ Done (Iteration 11, F18-P)** | DRAFT_ITER-10 D10-A | M | High — quantitative evidence of acceptable response times |
| 20 | ~~Error UX: user-friendly error messages with error-code → copy mapping in frontend~~ **✅ Done (Iteration 11, F18-O)** | DRAFT_ITER-10 D10-B | M | High — directly affects perceived quality |
...
| 22 | ~~API documentation: auto-generated OpenAPI spec + Swagger UI embed~~ **✅ Done (Iteration 11, F18-C)** | DRAFT_ITER-10 D10-D | M | High — standard evaluator expectation for a REST API |
...
19. **~~Latency benchmarks.~~** **✅ Done (Iteration 11, F18-P).** `test_endpoint_latency.py` with `pytest-benchmark` measuring P50/P95 for list, get, and upload endpoints. P95 < 500ms threshold enforced. Baseline numbers reproducible in CI.
20. **~~Error UX mapping.~~** **✅ Done (Iteration 11, F18-O).** `errorMessages.ts` with regex-based pattern matching maps 8 error patterns to localized Spanish user-friendly messages. Upload failures, extraction errors, and validation rejections all show clear copy.
...
22. **~~OpenAPI auto-generation.~~** **✅ Done (Iteration 11, F18-C).** Route tags, error response schemas (`ErrorResponse` model), and health response model added. `/docs` renders interactive API explorer with organized endpoints.
...
## E2E progress note (Iteration 11)

E2E coverage expanded from **5 tests (5 files)** to **20 tests (8 spec files)**, covering Phases 0–2 of the E2E coverage plan. Remaining: Phase 3 extended tests (46 tests) deferred to future iterations.

---

This roadmap was generated from audit findings (F1-A, F2-A, F4-A/B, F5-A), architecture decision records (ADR-ARCH-0001 through 0004), TECHNICAL_DESIGN.md deferred items, and DRAFT_ITER-10 triage (2026-02-26). Updated after Iteration 11 (2026-02-27) to reflect 9 newly resolved items: env reads (F18-A), PdfViewer mock (F18-B), OpenAPI (F18-C), failure-path tests (F18-Q), SourcePanel/UploadDropzone tests (F18-R), repo split (F18-S), latency benchmarks (F18-P), error UX (F18-O), AddFieldDialog tests (F18-R).