...
| 7b | Decompose `AppWorkspace.tsx` (~6 100 LOC) into ReviewWorkspace, StructuredDataView, PdfViewerContainer | [CTO_REVIEW_VERDICT.md](refactor/CTO_REVIEW_VERDICT.md), [codebase_audit.md](refactor/codebase_audit.md) | L | Medium |
...
7b. **AppWorkspace decomposition.** Split `frontend/src/AppWorkspace.tsx` (~6 100 LOC) into cohesive modules: `ReviewWorkspace` (interpretation/edit flow), `StructuredDataView` (canonical sections + field rows), and `PdfViewerContainer` (PDF rendering + annotations). The file was deprioritized during Phase 2 which targeted the three most impactful monoliths (`App.tsx`, `processing_runner.py`, `document_service.py`). **Acceptance criteria:** each extracted module stays under 500 LOC, all existing tests pass without modification, and `AppWorkspace.tsx` reduces to a layout shell below the cap.
