...
| 2 | Add CI coverage threshold gates | [codebase_audit.md](refactor/codebase_audit.md), [F4-A/F4-B in history](implementation/IMPLEMENTATION_HISTORY.md) | S | Low |
...
| 24 | CI/CD optimization: dependency caching, Python version matrix, deploy preview | DRAFT_ITER-10 D10-J | M | Low — internal DX, not user-visible |
...
24. **CI dependency caching + matrix.** Enable `npm ci` and `pip` caching in GitHub Actions, add Python 3.11 + 3.12 matrix. **Acceptance criteria:** CI wall-clock time reduced ≥30%; suite passes on both Python versions.

This roadmap was generated from audit findings (F1-A, F2-A, F4-A/B, F5-A), architecture decision records (ADR-ARCH-0001 through 0004), TECHNICAL_DESIGN.md deferred items, and DRAFT_ITER-10 triage (2026-02-26).