...
- [ ] F18-D 🔄 — Add 17 missing `data-testid` attributes to UI components (Codex) → source: F17-A
...
- [ ] F18-O 🔄 — Error UX mapping: `errorCodeMap` + user-friendly toast messages (Codex)
- [ ] F18-P 🔄 — Latency benchmarks: P50/P95 for upload, list, extract endpoints (Codex)

#### Phase D — Testing depth + architecture

- [ ] F18-Q 🔄 — Backend failure-path tests: orchestrator partial failures + DB lock/retry (Codex)
- [ ] F18-R 🔄 — SourcePanel + UploadDropzone test depth: branch coverage + interaction edge cases (Codex)
