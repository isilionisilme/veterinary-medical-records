...
| 7 | Concurrency | backend/app/infra/database.py | SQLite connections use defaults; no explicit busy_timeout/WAL strategy. | Lock contention risk under bursty processing/reprocess scenarios; reliability concern. | M | Connection config includes deterministic lock-handling policy (timeout + documented behavior) and tests cover lock scenario handling. |
