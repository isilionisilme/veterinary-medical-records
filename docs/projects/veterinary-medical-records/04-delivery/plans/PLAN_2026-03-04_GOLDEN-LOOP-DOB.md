...
- [x] P3-D 🔄 — Post-gate: ajustar `MIN_EXACT_MATCH_RATE` al valor alcanzado menos 5 pp, actualizar guardrails de `dob` en `extraction-quality.md`, añadir a Golden Fields status (GPT-5.3-Codex) — ✅ `d031b4fe`
- [x] CT-5 🔄 — Commit task: scope P3-D → `docs(plan-p3): dob threshold lock and extraction-quality update` → push (GPT-5.3-Codex) — ✅ `d031b4fe`
