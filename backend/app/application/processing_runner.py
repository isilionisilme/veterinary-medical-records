...
async def _process_document(
    *,
    ...
    quality_score, quality_pass, quality_reasons = _evaluate_extracted_text_quality(raw_text)
    logger.info(
        (
...
def _looks_human_readable_text(text: str) -> bool:
    ...
    if letters / len(text) < 0.35:
        return False
    # Reject chunks with too many symbol/control-like characters.
    strange = sum(
...
def _is_usable_extracted_text(text: str) -> bool:
    _, quality_pass, _ = _evaluate_extracted_text_quality(text)
    return quality_pass


def _evaluate_extracted_text_quality(text: str) -> tuple[float, bool, list[str]]:
    normalized = _normalize_candidate_text(text)
    if len(normalized) < 20:
        return 0.0, False, ["TOO_SHORT"]

    if not _looks_human_readable_text(normalized):
        return 0.0, False, ["NOT_HUMAN_READABLE"]

    # Require at least a minimal amount of word-like tokens so we don't
    # accept mostly-symbol noise as "completed extraction".
