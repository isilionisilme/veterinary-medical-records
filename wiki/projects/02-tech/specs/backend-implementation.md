...
> This document is **implementation guidance**, not a design authority. It translates the
> [Technical Design](technical-design) into backend responsibilities.
> | Part               | Sections                               | Focus                                                   |
> | ------------------ | -------------------------------------- | ------------------------------------------------------- |
> | **Architecture**   | Architecture, Persistence, Filesystem  | Module layering, SQLite model, filesystem paths.        |
> | **Execution**      | Processing execution model, Step model | Async runner, scheduler, retry/timeout, step tracking.  |
> | **Data contracts** | Structured interpretation schema, API  | Schema storage, calibration, review events, API wiring. |
> | **Quality**        | Logging, Testing                       | Structured logs, unit/integration test expectations.    |
> **Authority boundaries:**
> - System design, contracts, and invariants → [technical-design.md](technical-design) (Appendices A–E)
> - Product meaning → [product-design.md](product-design)
> - Frontend implementation → [frontend-implementation.md](frontend-implementation)
...
Use a modular-monolith hexagonal (ports-and-adapters) architecture:
...
graph TD
    subgraph "backend/app/"
        API["api/<br>FastAPI routers, error mapping"]
        APP["application/<br>use cases, orchestration"]
        DOM["domain/<br>entities, invariants"]
        PRT["ports/<br>repo/storage/runner interfaces"]
        INF["infra/<br>SQLite repos, filesystem, scheduler"]
    end
    ...
    style DOM fill:#eef4ff,stroke:#4f6df5,color:#1f2937
    style PRT fill:#eef4ff,stroke:#4f6df5,color:#1f2937
    style INF fill:#eef4ff,stroke:#4f6df5,color:#1f2937
```
...
Authority: [`docs/projects/veterinary-medical-records/02-tech/technical-design.md`](technical-design) Appendix B2
(Minimal Persistent Data Model).
...
Authority: [`docs/projects/veterinary-medical-records/02-tech/technical-design.md`](technical-design) Appendix A
(Contracts, States & Invariants).
...
Authority: [`docs/projects/veterinary-medical-records/02-tech/technical-design.md`](technical-design) Appendix A2 +
Appendix B1.2.
...
Authority: [`docs/projects/veterinary-medical-records/02-tech/technical-design.md`](technical-design) Appendix B1.2.1
(Persistence-Level Guard Pattern).
...
Authority: [`docs/projects/veterinary-medical-records/02-tech/technical-design.md`](technical-design) Appendix B5
(Filesystem Management Rules).
...
Authority: [`docs/projects/veterinary-medical-records/02-tech/technical-design.md`](technical-design) Appendix B3.2 +
Appendix B5.
...
Authority: [`docs/projects/veterinary-medical-records/02-tech/technical-design.md`](technical-design) Appendix B1.5 +
B1.5.1.
...
- Avoid event-loop blocking:
  - if a step performs blocking I/O or CPU-heavy work (e.g., PDF parsing), run it off the event loop (threadpool) so
...
Authority: [`docs/projects/veterinary-medical-records/02-tech/technical-design.md`](technical-design) Appendix B1.3 +
Appendix C3.

### Retry + timeout policy

Authority: [`docs/projects/veterinary-medical-records/02-tech/technical-design.md`](technical-design) Appendix B1.4 +
B1.4.1.
...
Authority: [`docs/projects/veterinary-medical-records/02-tech/technical-design.md`](technical-design) Appendix C1.1
(StepName).
...
- Payload schema is authoritative:
  [`docs/projects/veterinary-medical-records/02-tech/technical-design.md`](technical-design) Appendix C1.3.

### Failure mapping

Authority: [`docs/projects/veterinary-medical-records/02-tech/technical-design.md`](technical-design) Appendix C3
(Error Codes and Failure Mapping).

## Structured interpretation schema

Authority: [`docs/projects/veterinary-medical-records/02-tech/technical-design.md`](technical-design) Appendix D
(Structured Interpretation Schema visit-grouped canonical contract). Product semantics for confidence are defined in
[`docs/projects/veterinary-medical-records/01-product/product-design.md`](product-design); UX behavior remains in
...
- Backend does not backfill missing keys for presentation; frontend materializes the full schema view per Product Design
  authority.
...
- [`docs/projects/veterinary-medical-records/02-tech/technical-design.md`](technical-design) Appendix D3
  (Relationship to Persistent Model)
- [`docs/projects/veterinary-medical-records/02-tech/technical-design.md`](technical-design) Appendix B2.4
  (InterpretationVersion)
- [`docs/projects/veterinary-medical-records/02-tech/technical-design.md`](technical-design) Appendix A3
  (Interpretation & Versioning Invariants)
...
`StructuredField.is_critical` MUST be derived from `key ∈ CRITICAL_KEYS`. Source of truth:
...
- Compute Context during interpretation/review event handling and derive `context_key` as a stable canonical
  serialization/hash.
- Context-key serialization is stable across runs (for example, prefix with `ctx:` before serialization/hash).
- `mapping_id` originates from the extraction/mapping strategy used for each field result and is persisted with the
  field payload used for calibration events.
- `candidate_confidence` is diagnostic and is exposed only via debug endpoints/flags, never by default veterinarian UI
  endpoints.
...
- Load thresholds from versioned `config/confidence_policy.yaml` and persist/emit the active policy version with
  calibration updates.
- Structured logs for calibration updates must include: `document_id`, `run_id`, `context_key`, `field_key`,
  `mapping_id`, old/new `field_mapping_confidence`, and policy-state transitions.
...
- `field_review_history_adjustment` is sourced from calibration aggregates for (`context_key`, `field_key`,
  `mapping_id`) under the active `policy_version`; when no history exists, expose deterministic `0`.
- `field_mapping_confidence` must be composed as
  `clamp01(field_candidate_confidence + field_review_history_adjustment / 100)`.
...
- Default list exclusion of reviewed items is a UI behavior; backend exposes persisted reviewed state and timestamps
  consistently.
...
- Endpoint map: [`docs/projects/veterinary-medical-records/02-tech/technical-design.md`](technical-design) Appendix
  B3 (+ B3.1)
- Run resolution per endpoint:
  [`docs/projects/veterinary-medical-records/02-tech/technical-design.md`](technical-design) Appendix B3.1
- Error response format: [`docs/projects/veterinary-medical-records/02-tech/technical-design.md`](technical-design)
  Appendix B2.6
- Endpoint error semantics & error codes:
  [`docs/projects/veterinary-medical-records/02-tech/technical-design.md`](technical-design) Appendix B3.2
...
- When returning `409 CONFLICT`, ensure `details.reason` uses the Technical Design closed set (e.g., `NO_COMPLETED_RUN`,
  `REVIEW_BLOCKED_BY_ACTIVE_RUN`).

### Editing while active run exists

Authority: [`docs/projects/veterinary-medical-records/02-tech/technical-design.md`](technical-design) Appendix B3.2
(Conflict reasons and error semantics).

Implementation guidance:

- When a workflow is blocked by an active run, return the authoritative conflict response using
  `details.reason = REVIEW_BLOCKED_BY_ACTIVE_RUN`.

## Text extraction + language detection

### PDF extraction

Authority: [`docs/projects/veterinary-medical-records/02-tech/technical-design.md`](technical-design) (PyMuPDF
extraction).

Implementation note: if extracted text is empty/near-empty, the run may fail as `EXTRACTION_FAILED` (per Technical
Design step failure mapping; Appendix C3).

### Language detection

Authority: [`docs/projects/veterinary-medical-records/02-tech/technical-design.md`](technical-design) (langdetect
...
Authority: [`docs/projects/veterinary-medical-records/02-tech/technical-design.md`](technical-design) Appendix A8.1
(Event Type Taxonomy).

## Testing expectations

Authority: [`docs/projects/veterinary-medical-records/02-tech/technical-design.md`](technical-design) Appendix B7
...
- [`docs/projects/veterinary-medical-records/02-tech/technical-design.md`](technical-design) (including appendices),
  or
- [`docs/projects/veterinary-medical-records/04-delivery/implementation-plan.md`](implementation-plan) (scope and
  sequencing), or
- this document,
